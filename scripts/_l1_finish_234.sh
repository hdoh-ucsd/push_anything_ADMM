...
# Append-only: preserve seed=0/1 lines already in sweep.resume.summary
for SEED in 2 3 4; do
  echo "=== seed=$SEED start $(date +%s) ===" >> "$OUTDIR/sweep.resume.summary"
  echo "[df pre-seed=$SEED] $(df -h /tmp | tail -1)" >> "$OUTDIR/sweep.resume.summary"
  START=$(date +%s)
  ...
  echo "[df post-seed=$SEED] $(df -h /tmp | tail -1)" >> "$OUTDIR/sweep.resume.summary"
done
echo "DONE_234 $(date +%s)" >> "$OUTDIR/sweep.resume.summary"
touch "$OUTDIR/.resume_234_done"
