...
rm -f "$OUTDIR/.full_done"
for SEED in 0 1 2 3 4; do
  echo "=== seed=$SEED start $(date +%s) ===" >> "$OUTDIR/sweep.summary"
  START=$(date +%s)
  ...
  echo "seed=$SEED ${ELAPSED}s n_lcs=$N_LCS n_aee1=$N_AEE1 n_gate=$N_GATE | $RESULT_LINE" >> "$OUTDIR/sweep.summary"
done
echo "DONE $(date +%s)" >> "$OUTDIR/sweep.summary"
touch "$OUTDIR/.full_done"
