...
- **Tier 2 — reference-side deep read + port-side instrumented run, 2026-07-14:**
  ...
  - **Tier 3 correction (2026-07-28):** the Tier-1 coupling claim above is WRONG on both sides, in a way that cancels. Reference: `operational_space_control.cc:407` calls `UpdateDynamics(x_w_spr, active_contact_names, {})` — the empty third argument means `inverse_dynamics_qp.cc:207`'s `Je^T·λ_e` injection loop never executes; `λ_e` exists only in the tracking cost and **has zero effect on the solved τ**. Port: the force-tracking path likewise keeps `A_eq=[M,−B]` (dynamics decoupled; the 2026-07-19 phantom-slam fix). Both sides are cost-only shadows → **CONFORMANT, but the force target is torque-inert in both**. Consequence verified empirically (p126 vs p127, 2026-07-28): swapping `λ_des` routing (fabricated −g_hat vs u_sol recovery) produces no significant metric change under `use_force_tracking=True`; the reference architecture's entire push authority is position-trajectory tracking.
