...
The 9.4.5-B premise — that fixing executor home-hold would unblock verdict-A — is **falsified**. 9.4.5-D Candidate 2 sub-option 1a (`w_align` decay) is queued as the next fix attempt.
...
| 2.1 kIK guide-path (Effect A) + executor home-hold (Effect B) + wrapper-executor coupling (Effect C) | Effect B Attempt 1 PARTIAL (commit `161b7d9`); sub-attempt 1.5 reverted; Effect C CONFIRMED (9.4.5-C, step 8 Candidate 2 newly binding) | Binding constraint — three compounding effects |
