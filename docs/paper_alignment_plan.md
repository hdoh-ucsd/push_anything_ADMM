...
Status: **REOPENED (wrapper-side, post-F3)** — Finding A RESOLVED by F2 (2026-05-11); F3 shipped y_max 0.0→0.13 closes generation gap (2026-05-14); Effect C persistence (kToReposCost exit) now the standalone binding constraint. 1a/1b/1c still candidates for re-evaluation against post-F3 landscape; 1d REFINED-FALSIFIED. See journal 2026-05-12, 2026-05-13, 2026-05-14.
...
**Post-F3 caveat.** The 9.4.7-B/C measurements above (cost-landscape characterization, 1a/1b/1c re-evaluations) were taken *pre-F3-ship* under the CLI override. The post-F3 landscape (with workspace_xy_max[1] = 0.13 in both dataclass default and shipped yaml) has not been characterized for 1a/1b/1c — the F3 Phase 2 sweep showed M2 stratifies at 47.5% Path D / 0.0% Path A, which already invalidates the "strat_0 wins WHEN generated" premise from journal 2026-05-13 on Path A. Re-derive 1a/1b/1c rationales against post-F3 c_C3_raw / align_bonus / travel breakdowns before treating them as actionable.

**F3 SHIPPED — workspace y-bound relaxation.** `workspace_xy_max[1] = 0.0 → 0.13` (matches `sampling_radius`). See journal 2026-05-14. Phase 2 sweep (six y_max points × two paths) confirmed: M1 (strat_0 generation) climbed 17% → 100% (Path D saturates at y=0.05, Path A at y=0.10); M3 (downstream object motion) bit-identical across y_max ∈ {0.10, 0.13, 0.15, 0.20} on both paths and shows no improvement toward goal. F3 is necessary, not sufficient. Persistence (kToReposCost-driven c3-mode exit within 1–17 steps) is the next binding constraint, surfaced first by 9.4.7 Option A (under override) and now confirmed standalone post-F3-ship. Original F3 enumeration cross-link: `control/sampling_c3/sampling.py:170-180`.
...
| 2.1 kIK guide-path (Effect A) + executor home-hold (Effect B) + wrapper-executor coupling (Effect C) | **REOPENED (wrapper-side, post-F3)** — Finding A RESOLVED by F2 (2026-05-11); F3 SHIPPED 2026-05-14 (`workspace_xy_max[1]` 0.0→0.13) closes generation gap; Effect C persistence (kToReposCost-driven exit) now standalone binding constraint; 1d REFINED-FALSIFIED, 1a/1b/1c need re-derivation under post-F3 landscape; G5 deferred. See journal 2026-05-12, 2026-05-13, 2026-05-14. | Binding constraint — geometric + generation mechanisms closed; wrapper-arbitration persistence is the binding surface |
